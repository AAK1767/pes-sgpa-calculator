...
export const getGradePoint = (totalMarks, subject = null, fallbackGradeMap = GradeMap) => {
  const map = (subject && subject.customGradeMap) ? subject.customGradeMap : fallbackGradeMap;
  for (let g of map) {
...
export const getGradeInfo = (score, subject = null, fallbackGradeMap = GradeMap) => {
  const map = (subject && subject.customGradeMap) ? subject.customGradeMap : fallbackGradeMap;
  return map.find(g => score >= g.min) || map[map.length - 1];
};

export const getRequiredESAForGrade = (subject, targetScore, withSafetyMargin = true, options = {}, marks) => {
  const {
...
export const getRequiredISA2ForGrade = (subject, targetScore, options = {}, marks) => {
...
export const getRequiredISA2ForPass = (subject, marks) => {
  return getRequiredISA2ForGrade(subject, 40, { assumeFullForEmptyInternals: true }, marks);
};
